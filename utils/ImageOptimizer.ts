...
const getImageFromWeb = async({
  ...
  await streamPipeline(response.body, createWriteStream(`./tmp/${name}-${index}.png`));
...
const converter = async({images, name}:converterProps):Promise<string[]> => {
  ...
  if (images_url) {
    for( const [index, image_url] of images_url.entries() ) {
      ...
      if (!flag) {
        ...
        const file = await fs.readFile(process.cwd() + fetch_image_path);
