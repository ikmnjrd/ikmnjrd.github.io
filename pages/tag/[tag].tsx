...
export default function TagLinkPage({
  ...
  return (
    <>
      ...
      <h2>{ tag }の投稿記事一覧</h2>
      <div>
        { posts.map((post:any) => <li key={post.slug}><Link href={`/blog/${post.slug}`}><a className="hover:underline hover:text-newmo-400 visited:text-newmo-300">{post.frontmatter.title}</a></Link></li>) }
      </div>
    </>
  )
}
